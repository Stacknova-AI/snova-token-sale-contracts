...
describe("PresaleSNOVA Contract - Referral Counting Functionality", function () {
    ...
    it("Should return correct referral count", async function () {
        ...
        await transferAndApproveTokens(amounts, user1)

        await tokenSaleRegistry.grantRole(ethers.id("OPERATOR_ROLE"), presale)

        await presale.connect(user2).purchaseTokens(user3.address, erc20MockStable.target, ethers.parseEther("50"))
        await presale.connect(user1).purchaseTokens(user3.address, erc20MockStable.target, ethers.parseEther("57"))
        await presale.connect(user2).purchaseTokens(user3.address, erc20MockStable.target, ethers.parseEther("140"))

        const referralCount = await presale.getReferralCount(user3)
        expect(referralCount).to.equal(2)
    })

    it("Should return correct referral count for native currency", async function () {
        await configureAndStartSaleRound()

        await tokenSaleRegistry.grantRole(ethers.id("OPERATOR_ROLE"), presale)
        await presale
            .connect(owner)
            .purchaseTokens(user3.address, NATIVE_CURRENCY_ADDRESS, 0, { value: ethers.parseEther("0.016") })
        await presale
            .connect(user1)
            .purchaseTokens(user3.address, NATIVE_CURRENCY_ADDRESS, 0, { value: ethers.parseEther("0.017") })
        await presale
            .connect(user2)
            .purchaseTokens(user3.address, NATIVE_CURRENCY_ADDRESS, 0, { value: ethers.parseEther("0.018") })
        await presale
            .connect(user2)
            .purchaseTokens(user3.address, NATIVE_CURRENCY_ADDRESS, 0, { value: ethers.parseEther("0.016") })
